document.addEventListener("DOMContentLoaded",function(){
    ...
    const statsContainer=document.getElementsByClassName("stats_container");
    const easyProgresssCircle=document.getElementsByClassName("easy_progress");
    const mediumProgresssCircle=document.getElementsByClassName("medium_progress");
    const hardProgresssCircle=document.getElementsByClassName("hard_progress");
    ...
    const cardStatsContainer=document.getElementsByClassName("stats_card");
    ...
    try{
        ...
        const targeturl = `http://localhost:4000/leetcode-proxy`;  // <--- use local proxy now
...
//         const cardData = [
//   { label: "Overall Submissions", value: parseddata.data.matchedUser?.submitStats.totalSubmissionNum[0].submissions || "user data not available" },
//   { label: "Overall Easy Submissions", value: parseddata.data.matchedUser?.submitStats.totalSubmissionNum[1].submissions || "user data not available"  },
//   { label: "Overall Medium Submissions", value: parseddata.data.matchedUser?.submitStats.totalSubmissionNum[2].submissions || "user data not available"  },
//   { label: "Overall Hard Submissions", value: parseddata.data.matchedUser?.submitStats.totalSubmissionNum[3].submissions || "user data not available"  }
// ];
...
function displayUserData(parseddata) {
    if (parseddata.data.matchedUser === null) {
        statsContainer.innerHTML = "No Data Found.";
        cardStatsContainer.innerHTML = "";  // Also clear cards
        return;
    }

    // ✅ Clear "No Data Found" message when valid data is received
